...
namespace utils::hash
{
	namespace
	{
		...
		{
			if (file_size < sizeof(XPakHeader))
			{
				return {};
			}

			XPakHeader header{};
			file_stream.read(reinterpret_cast<char*>(&header), sizeof(XPakHeader));

			constexpr auto hash_size = sizeof(DB_AuthHash);

			DB_AuthHash empty_hash{};
			if (!std::memcmp(header.hash.bytes, empty_hash.bytes, hash_size))
			{
				printf("Computing pakfile hash because its missing, this may take some time...\n");
				...
				auto bytes_to_read = file_size - sizeof(XPakHeader);
				...
				out_stream.write(reinterpret_cast<char*>(&header), sizeof(XPakHeader));
			}
		...
		{
			if (buffer.size() < sizeof(XPakHeader))
			{
				return {};
			}

			constexpr auto hash_size = sizeof(DB_AuthHash);
			const auto header = reinterpret_cast<XPakHeader*>(buffer.data());

			DB_AuthHash empty_hash{};
			if (!std::memcmp(header->hash.bytes, empty_hash.bytes, hash_size))
			{
				printf("Computing pakfile hash because its missing, this may take some time...\n");
				const auto hash_start = reinterpret_cast<std::uint8_t*>(buffer.data() + sizeof(XPakHeader));
				const auto len = buffer.size() - sizeof(XPakHeader);
	...
	{
		if (filename.ends_with(".pak") || filename.ends_with(".xpak"))
		{
